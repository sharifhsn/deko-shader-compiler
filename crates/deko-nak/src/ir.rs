...
impl Src {
    ...
    pub fn is_fneg_zero(&self, src_type: SrcType) -> bool {
        match self.as_u32(src_type) {
            ...
            Some(0x80000000) => src_type == SrcType::F32 || src_type == SrcType::F64,
...
impl Instr {
    ...
    pub fn ftz(&self) -> bool {
        match &self.op {
            ...
            Op::HSet2(op) => op.ftz,
            Op::HSetP2(op) => op.ftz,
            Op::MuFu(op) => op.op_type == FloatType::F32 && op.op != MuFuOp::Tanh,
...
macro_rules! sm_match {
    ($self: expr, |$x: ident| $y: expr) => {{
        assert!(
            (50..60).contains(&$self.sm),
            "only Maxwell shader models are supported"
        );
