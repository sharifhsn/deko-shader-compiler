...
fuzz_target!(|bytes: &[u8]| {
    ...
    let source = match stage {
        ...
        Stage::Compute if bytes[2] % 5 == 0 => format!(
        ...
        Stage::Compute if bytes[2] % 5 == 1 =>
            "@compute @workgroup_size(32) fn main() { subgroupBarrier(); }".to_owned(),
        Stage::Compute if bytes[2] % 5 == 2 => "@compute @workgroup_size(32) fn main(@builtin(local_invocation_index) lane: u32) { _ = subgroupBroadcastFirst(lane); }".to_owned(),
        Stage::Compute if bytes[2] % 5 == 3 => "@compute @workgroup_size(40) fn main(@builtin(subgroup_invocation_id) lane: u32, @builtin(subgroup_size) size: u32, @builtin(subgroup_id) subgroup: u32, @builtin(num_subgroups) count: u32) { _ = lane + size + subgroup + count; }".to_owned(),
        Stage::Compute => "@compute @workgroup_size(32) fn main(@builtin(local_invocation_index) lane: u32) { let predicate = lane < 16u; _ = subgroupAll(predicate); _ = subgroupAny(predicate); _ = subgroupBallot(predicate); }".to_owned(),
    };
    Compiler
        .compile_wgsl(
            &source,
