...
fuzz_target!(|bytes: &[u8]| {
    ...
    let source = match stage {
        ...
        Stage::Compute if bytes[2] % 8 == 0 => format!(
        ...
        Stage::Compute if bytes[2] % 8 == 1 =>
            "@compute @workgroup_size(32) fn main() { subgroupBarrier(); }".to_owned(),
        Stage::Compute if bytes[2] % 8 == 2 => "@compute @workgroup_size(32) fn main(@builtin(local_invocation_index) lane: u32) { _ = subgroupBroadcastFirst(lane); }".to_owned(),
        Stage::Compute if bytes[2] % 8 == 3 => "@compute @workgroup_size(40) fn main(@builtin(subgroup_invocation_id) lane: u32, @builtin(subgroup_size) size: u32, @builtin(subgroup_id) subgroup: u32, @builtin(num_subgroups) count: u32) { _ = lane + size + subgroup + count; }".to_owned(),
        Stage::Compute if bytes[2] % 8 == 4 => "@compute @workgroup_size(32) fn main(@builtin(local_invocation_index) lane: u32) { let predicate = lane < 16u; _ = subgroupAll(predicate); _ = subgroupAny(predicate); _ = subgroupBallot(predicate); }".to_owned(),
        Stage::Compute if bytes[2] % 8 == 5 => "@compute @workgroup_size(7) fn main(@builtin(local_invocation_index) lane: u32) { let value = lane + 1u; _ = subgroupAdd(value); _ = subgroupXor(value); _ = subgroupExclusiveAdd(value); _ = subgroupInclusiveMul(value); }".to_owned(),
        Stage::Compute if bytes[2] % 8 == 6 => "@group(0) @binding(0) var<storage, read_write> output: array<u32>; @compute @workgroup_size(4) fn main(@builtin(local_invocation_index) lane: u32) { if lane == 0u { return; } output[lane] = 7u; }".to_owned(),
